...
The data also contains two nucleotide sequences: \Robject{mrna} and \Robject{genomic}. The first one is the mRNA sequence of FN1, an isoform known as fibronectin 1 isoform 2 preproprotein. FN1 has at least 16 different known isoforms. The sequence was loaded by a call to \Rfunction{readFASTA} from the package \Rpackage{Biostrings}, using a regular FASTA-formatted text file as argument.  The second sequence is the DNA sequence of the gene, with one FASTA entry per exon. We will return to that later.
